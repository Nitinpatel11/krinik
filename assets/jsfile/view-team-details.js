...
document.addEventListener('DOMContentLoaded', async function () {
  try {
    ...
    const teamResponse1 = await fetch(`https://krinik.pythonanywhere.com/league_get/`);
    const teamResponse2 = await fetch(`https://krinik.pythonanywhere.com/player_get/`);
...
return player.team_name === specificTeam.team_name && 
leagueData.some(league => league.league_name === specificTeam.league_name);
});
console.log(filteredPlayers)
    ...
    if (filteredPlayers ) {
      ...
      if (specificTeam.team_image) {
        teamImagePreview.src = 'https://krinik.pythonanywhere.com' + specificTeam.team_image;
      } else {
        teamImagePreview.src = ''; // Clear image source if no image available
      }

        startTeamDateInput.value = moment(specificTeam.team_date,'YYYY-MM-DD').format('DD-MM-YYYY');
