...
const userTransactionHistory = document.getElementById("user-transaction-history");
...
const matchingData = addAmount.find(item => item.id === response1);
if (!matchingData) {
...
playerId.value = matchingData.Player_ID;
playerName.value = matchingData.Player_Name;
playerTotalAmount.value = matchingData.Total_Play_Coin;
playerTotalWinning.value = matchingData.Total_Winning_Coin;
playerId1.value = matchingData.Player_ID;
playerName1.value = matchingData.Player_Name;
...
const playerTotalAmount = document.getElementById("player-total-amount").value;
const playerTotalWinning = document.getElementById("player-total-winning").value;
const playerAddAmount = document.getElementById("player-add-amount").value;
const playerWinningAmount = document.getElementById("player-winning-amount").value;
...
const amount1 = parseFloat(playerTotalAmount) + parseFloat(playerAddAmount || 0);
const amount2 = parseFloat(playerTotalWinning) + parseFloat(playerWinningAmount || 0);

const matchUrl = `https://krinik.pythonanywhere.com/add_wallet/${id}`;
...
const playerTotalAmount = document.getElementById("player-total-amount").value;
const playerTotalWinning = document.getElementById("player-total-winning").value;
const playerDeductAmount = document.getElementById("deductAmountInPlay").value;
...
const amount1 = parseFloat(playerTotalAmount) - parseFloat(playerDeductAmount || 0);
const amount2 = parseFloat(playerTotalWinning) - parseFloat(playerWinningAmount1 || 0);
...
document.getElementById("user-unlock").addEventListener("click", () => lock());
// Event handlers for buttons
document.getElementById("user-transaction-history").addEventListener("click", () => redirectToHistoryPage('user-transaction-history'));
document.getElementById("user-game-history").addEventListener("click", () => redirectToHistoryPage('user-pool-history'));
document.getElementById("user-withdraw").addEventListener("click", () => redirectToHistoryPage('user-withdraw-history'));
